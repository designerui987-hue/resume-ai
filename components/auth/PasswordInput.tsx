import React, { useState, forwardRef } from 'react';
import { Eye, EyeOff, Lock } from 'lucide-react';

interface PasswordInputProps extends React.InputHTMLAttributes<HTMLInputElement> {
  label?: string;
  error?: string;
  showLockIcon?: boolean;
  rightLabelAction?: React.ReactNode;
}

export const PasswordInput = forwardRef<HTMLInputElement, PasswordInputProps>(
  ({ label = 'Password', error, showLockIcon = false, rightLabelAction, className = '', ...props }, ref) => {
    const [showPassword, setShowPassword] = useState(false);

    return (
      <div className="space-y-1.5 w-full">
        <div className="flex items-center justify-between">
          <label className="block text-xs font-semibold text-[#18181B]">
            {label}
          </label>
          {rightLabelAction}
        </div>

        <div className="relative flex items-center">
          <input
            ref={ref}
            type={showPassword ? 'text' : 'password'}
            {...props}
            className={`w-full px-3.5 py-2.5 rounded-xl bg-[#FAFAF9] border text-xs text-[#18181B] placeholder:text-[#A1A1AA] transition-colors focus:outline-none focus:bg-white focus:border-[#111827] focus:ring-1 focus:ring-[#111827] ${
              error ? 'border-[#B91C1C]' : 'border-[#E4E4E7]'
            } ${showLockIcon ? 'pr-16' : 'pr-10'} ${className}`}
          />

          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5 text-[#71717A]">
            {showLockIcon && <Lock className="w-3.5 h-3.5 text-[#71717A]" />}
            <button
              type="button"
              onClick={() => setShowPassword((p) => !p)}
              className="p-1 hover:text-[#18181B] transition-colors cursor-pointer"
              title={showPassword ? 'Hide password' : 'Show password'}
              aria-label={showPassword ? 'Hide password' : 'Show password'}
            >
              {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            </button>
          </div>
        </div>
        {error && <p className="text-[11px] font-medium text-[#B91C1C]">{error}</p>}
      </div>
    );
  }
);

PasswordInput.displayName = 'PasswordInput';
